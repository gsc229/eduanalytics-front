...
const normalizeData = (schools: any) => {
  const nomralizedSchools = schools.map((school: any) => {
    const normalizedSchool = {
      id: null,
      school: null,
      program_percentage: null,
      race_ethnicity: null,
      size: null,
    };
    normalizedSchool.id = school.id;
    normalizedSchool.school = school.school;
    normalizedSchool.program_percentage =
      school.latest.academics.program_percentage;
    (normalizedSchool.race_ethnicity =
...
export const useSchools = (initial: SchoolDataType[]) => {
  ...
  const [currentSchool, setCurrentSchool] = React.useState<
    SchoolDataType | undefined
  >(getLSCurrentSchool());
  const [drawerOpen, setDrawerOpen] = React.useState(false);
  const [componentRef, setComponentRef] =
    React.useState<React.MutableRefObject<null>>();
